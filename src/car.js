...
embox2dTest_car.prototype.setup = function() {
  ...
  // Boxes
  {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      World.world.CreateBody(bd).CreateFixture(box, 0.5);
    }
  }
  ...
  //car
  {
    ...
    this.carBody = World.world.CreateBody(bd);
    ...
    var wheelBody1 = World.world.CreateBody(bd);
    wheelBody1.CreateFixture(fd);

    bd.set_position( new b2Vec2(1.0, 0.4) );
    var wheelBody2 = World.world.CreateBody(bd);
    wheelBody2.CreateFixture(fd);

    /* bd.set_position( new b2Vec2(0.0, 0.4) );
       var wheelBody3 = World.world.CreateBody(bd);
       wheelBody3.CreateFixture(fd); */
    ...
    this.rearWheelJoint = Box2D.castObject( World.world.CreateJoint(jd), b2WheelJoint );

    jd.Initialize(this.carBody, wheelBody2, wheelBody2.GetPosition(), axis);
    jd.set_motorSpeed(0.0);
    jd.set_maxMotorTorque(30.0);
    jd.set_enableMotor(false);
    jd.set_frequencyHz(m_hz);
    jd.set_dampingRatio(m_zeta);
    this.wheelJoint2 = Box2D.castObject( World.world.CreateJoint(jd), b2WheelJoint );
  }
}
...
export default embox2dTest_car;
